...
The Worker also accepts the future `https://www.packardfor64.com` origin and gives Stripe the matching `/donate/return/` URL, while continuing to support the current GitHub Pages origin and `/packardfor64/` path. This is preparation only: the repository has no `docs/CNAME` yet, and GoDaddy DNS still points to the former website. Configure the custom domain in GitHub Pages before changing GoDaddy's web records. After the DNS switch, retest checkout and activate/verify FormSubmit for the new site origin if it requests that step.
