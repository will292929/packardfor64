...
The Worker also accepts `https://packardfor64.com` and `https://www.packardfor64.com` and gives Stripe the matching `/donate/return/` URL, while continuing to support the current GitHub Pages origin and `/packardfor64/` path. GitHub Pages now has `packardfor64.com` in [`docs/CNAME`](docs/CNAME), but GoDaddy DNS still points to the former website. After the DNS switch, retest checkout and activate/verify FormSubmit for the new site origin if it requests that step.
...
The supplied Text Message Terms and Privacy Policy have been adapted to the site's current capabilities and published at `/text-terms/` and `/privacy/` in the GitHub Pages build. The Campaign should review factual data practices and text-program operations before relying on these pages for registration or compliance. GitHub Pages has the custom domain configured, but GoDaddy DNS has not yet switched to GitHub. Verify the custom domain before using its URLs for campaign materials.
